...
* [`src/drone_lintu/styles.cljs`](src/drone_lintu/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/drone_lintu/`](src/drone_lintu/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/drone_lintu/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `drone-lintu` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/drone_lintu/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns drone-lintu.example
  (:require [drone-lintu.config :as config])
